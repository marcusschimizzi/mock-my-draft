...
must_haves:
  truths:
    - 'Import pipeline produces a validated data version without replacing the active version on failure.'
    - 'Manual import endpoint triggers an import and returns status details for the latest run.'
    - 'Daily scheduled import runs automatically and logs success or failure outcomes.'
  artifacts:
